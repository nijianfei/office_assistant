...
if __name__ == "__main__":
    ...
    logger.debug("=== 试用管理器测试 ===")
    logger.debug(f"机器指纹: {info['fingerprint']}")
    logger.debug(f"首次启动: {info['first_start_time']}")
    logger.debug(f"上次运行: {info['last_run_time']}")
    logger.debug(f"剩余天数: {info['remaining_days']}")
    logger.debug(f"剩余时长: {info['remaining_hours']} 小时")
    logger.debug(f"累计运行: {info['total_run_hours']} 小时")
    logger.debug(f"启动次数: {info['startup_count']}")
    logger.debug(f"是否有效: {info['is_valid']}")
